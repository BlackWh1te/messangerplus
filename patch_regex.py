import re

with open('app/chat/page.tsx', 'r', encoding='utf-8') as f:
    c = f.read()

# 1. State for Voice
if 'const [isRecording, setIsRecording] = useState(false)' not in c:
    c = re.sub(r'(const \[partnerTyping, setPartnerTyping\] = useState\(false\))', 
               r'\1\n  const [isRecording, setIsRecording] = useState(false)\n  const mediaRecorderRef = useRef<MediaRecorder | null>(null)\n  const audioChunksRef = useRef<Blob[]>([])', c)

# 2. Render Audio tag for [audio:...]
if '[audio:' not in c:
    render_msg_new = """    if (content.startsWith('[image:') && content.endsWith(']')) {
      const url = content.slice(7, -1)
      return <img src={`/api/proxy${url}`} alt="Uploaded image" className="max-w-full rounded-md object-contain max-h-64 cursor-pointer" />
    }
    if (content.startsWith('[audio:') && content.endsWith(']')) {
      const url = content.slice(7, -1)
      return <audio controls src={`/api/proxy${url}`} className="max-w-[200px] h-10 rounded-full" />
    }"""
    c = re.sub(r'if \(content\.startsWith\(\'\[image:\'\) && content\.endsWith\(\'\]\'\)\) \{.*?\n    \}', render_msg_new, c, flags=re.DOTALL)

# 3. Voice Recording Functions
if 'const startRecording = async () =>' not in c:
    fns_new = """  const startRecording = async () => {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true })
      const recorder = new MediaRecorder(stream)
      audioChunksRef.current = []
      
      recorder.ondataavailable = e => {
        if (e.data.size > 0) audioChunksRef.current.push(e.data)
      }
      
      recorder.onstop = async () => {
        stream.getTracks().forEach(t => t.stop())
        setIsRecording(false)
        const audioBlob = new Blob(audioChunksRef.current, { type: 'audio/webm' })
        if (audioBlob.size < 1000) return // too short
        
        try {
          setBanner('Uploading voice message...')
          const file = new File([audioBlob], 'voice.webm', { type: 'audio/webm' })
          const res = await uploadImage(tokenRef.current!, file as any)
          await sendMessageHttp(tokenRef.current!, `[audio:${res.url}]`)
          setBanner('')
        } catch (err: any) {
          setBanner('Voice upload failed')
        }
      }
      
      recorder.start()
      mediaRecorderRef.current = recorder
      setIsRecording(true)
    } catch (e) {
      setBanner('Microphone permission denied')
    }
  }

  const stopRecording = () => {
    if (mediaRecorderRef.current && isRecording) {
      mediaRecorderRef.current.stop()
    }
  }

  async function compressImage(file: File): Promise<File>"""
    c = re.sub(r'async function compressImage\(file: File\): Promise<File>', fns_new, c)

# 4. Sound & Vibration
if 'navigator.vibrate' not in c:
    ws_msg_new = """        setPartnerTyping(false)
        setMessages(prev => {
          if (prev.find(m => m.id === data.data.id)) return prev
          if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate(100)
          try {
            const audio = new window.Audio('data:audio/mp3;base64,//NExAAAAANIAAAAAExBTUUzLjEwMKqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq')
            audio.volume = 0.5
            audio.play().catch(()=>{})
          } catch(e){}
          return [...prev, data.data]
        })"""
    c = re.sub(r'setPartnerTyping\(false\) // clear typing on message\s*setMessages\(prev => \{\s*if \(prev\.find\(m => m\.id === data\.data\.id\)\) return prev\s*return \[\.\.\.prev, data\.data\]\s*\}\)', ws_msg_new, c, flags=re.DOTALL)

# 5. UI Microphone button
if 'isRecording ? stopRecording : startRecording' not in c:
    mic_btn_new = """            <button
              type="button"
              onClick={isRecording ? stopRecording : startRecording}
              className={`p-3 rounded-full transition-colors flex-shrink-0 ${isRecording ? 'bg-red-500 animate-pulse text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z"/>
                <path d="M19 10v2a7 7 0 0 1-14 0v-2"/>
                <line x1="12" y1="19" x2="12" y2="22"/>
              </svg>
            </button>
            <button
              type="submit"
              disabled={(!input.trim() && !isRecording) || isUploading}
              className="p-3 bg-teal-500 text-white rounded-full hover:bg-teal-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex-shrink-0"
            >"""
    c = re.sub(r'<button\s*type="submit"\s*disabled=\{\!input\.trim\(\) \|\| isUploading\}\s*className="p-3 bg-teal-500 text-white rounded-full hover:bg-teal-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex-shrink-0"\s*>', mic_btn_new, c, flags=re.DOTALL)


# 6. Typing UI
if 'partner is typing...' not in c:
    typing_ui = """        </CallLaunchButtons>
      </header>
      {partnerTyping && (
        <div className="bg-gray-800 text-gray-400 text-xs px-4 py-1 animate-pulse">
          partner is typing...
        </div>
      )}"""
    c = re.sub(r'</CallLaunchButtons>\s*</header>', typing_ui, c)

with open('app/chat/page.tsx', 'w', encoding='utf-8') as f:
    f.write(c)
print("Regex patch done")
